...
def add_update_tab():
    selected_date = st.date_input("Enter date", datetime(2024,8,1), label_visibility="collapsed").strftime("%Y-%m-%d")
    ...
    with st.form(key="expense_form"):
        ...
        for i in range(5):
            ...
            col1, col2, col3 = st.columns(3)
            with col1:
                number_input = st.number_input(label="Amount", min_value=0.0, step=1.0, value=amount, key=f"amount_{i}", label_visibility="collapsed")
            with col2:
